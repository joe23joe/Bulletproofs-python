...
def fold_points(point_vec, u):
    # your code here
    # called with u_inv, so internally u*GL + (1/u)*GR = u_inv*GL + u*GR
    n = len(point_vec)
    half = n // 2
    u_inv = pow(u, -1, p)
...
def compute_secondary_diagonal(G_vec, a):
    ...
    n = len(G_vec)
    half = n // 2
    L = vector_commit(G_vec[half:], a[:half])   # right points, left scalars  → u²
    R = vector_commit(G_vec[:half], a[half:])   # left  points, right scalars → u⁻²
    return L, R
